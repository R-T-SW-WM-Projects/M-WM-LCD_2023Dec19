...
{
    int cnt = 0;
    if (DMA_NODE_BUFFER_MAX_SIZE % 2 != 0) {
        ESP_LOGE(TAG, "ESP32 only supports 2-byte aligned data length");
        return ESP_FAIL;
    }
    if (max_dma_buffer_size >= DMA_NODE_BUFFER_MAX_SIZE * 2) {
        lcd_cam_obj->lcd.dma_node_buffer_size = DMA_NODE_BUFFER_MAX_SIZE;
 ...
 * 
 * @param handle i2s lcd driver handle to deinitialize
 * 
 * @return
 *      - ESP_OK on success
